...
export default function DashboardPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return goats.filter((g) => {
      const matchesSearch = [g.ID, g.Name].some((v) => v.toLowerCase().includes(query));
  ...
  return (
    <section className="space-y-4">
      ...
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by ID or name" className="w-full rounded-xl border border-farm-200 p-3" />
      ...
      <div className="space-y-3">{filtered.map((goat) => <GoatCard key={goat.ID + goat["QR Code"]} goat={goat} />)}</div>
